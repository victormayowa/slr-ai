...
async def _call_provider(prompt: str, provider: str, json_output: bool, max_tokens: int) -> str:
    ...
    if provider == "gemini":
        ...
        result = await gemini_client.aio.models.generate_content(model=GEMINI_MODEL, contents=prompt, config=config)
...
async def _complete(prompt: str, provider: str, json_output: bool, max_tokens: int) -> str:
    async with _semaphore:
        try:
            ...
            raise LLMError(f"The {provider} request failed. Check the API key, model name, and rate limits.") from exc
    if not text.strip():
